...
import { generateAccessTokenOnly, generateAuthenticationTokens, } from "../../utils/tokens.js";
...
export default class AuthController extends BaseController {
    ...
    async register(req, res) {
        const { pseudo, email, password, avatar, confirm } = await registerSchema.parseAsync(req.body);
        if (password !== confirm) {
            return res
                .status(400)
        ...
        const errors = alreadyExistingUser.reduce((acc, user) => {
            if (user.email === email)
                acc.email = "Email déjà utilisé";
            if (user.pseudo === pseudo)
                acc.pseudo = "Pseudo déjà utilisé";
            return acc;
        }, {});
...
async function generateAndSetTokens(res, user) {
...
async function replaceRefreshTokenInDatabase(refreshToken, user) {
...
async function deleteTokenAndCookies(req, res) {
    const rawToken = req.cookies?.refreshToken;
    if (rawToken) {
